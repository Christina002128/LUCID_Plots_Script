...
excel_file="../supplement_LUCID_baseline_INVAR2_ChristinaZ_3Mar.xlsx"
...
p1<- ggplot(df_plot, aes(x = Patient, y = Label, fill = factor(Value))) +
  ...
  labs(subtitle = "Patient",x = "", y = "Gene", fill = "Mutation") +
  theme_minimal() +
  theme(
...
p2<-ggplot(df_plot, aes(x = Patient, y =  Label, fill = factor(Value))) +
  ...
  scale_fill_manual(values = c("0" = "white", "1" = "#3FAB3E")) +
  scale_x_discrete(position = "top") +  # Moves the x axis to the top
  labs(x = "", y = "Gene", fill = "Mutation",subtitle = "Patient") +
  theme_minimal() +
  theme(
    axis.text.y = element_text(colour = "black",face = "italic",size=10),
    plot.subtitle = element_text(hjust = 0.5,vjust=-5,size=13),   # Center the title at the top
...
p2<-ggplot(detection_data, aes(x = PATIENT)) +
  ...
  labs(
    x = "Samples, ordered by ctDNA fraction",
...
design <- c(
  area(t = 1, l = 1, b = 1, r = 1),  # Placeholder for A (Row 1, Column 1)
...
combined_plot<-plot_spacer()+p_list[[1]]+ p_list[[2]] + p_list[[3]] + p_list[[4]] + p_list[[5]] + plot_layout(design = design,heights =c(4,4,1,1,1),widths = c(2,2,4,4,4,4))
...
ha1 <- HeatmapAnnotation(
  ...
  annotation_label = "ctDNA fraction",
...
ht <- Heatmap(matrix_data,  row_names_side = "left",heatmap_width = unit(20, "cm"),
          ...
          "Pathology","Treatment","Smoking status","Age","Sex","INVAR",expression("InVisionFirst"^"\u00AE"~"-Lung")),
        top_annotation = ha1,
...
p2<-ggplot(volume_plot, aes(x = T_volume_cm3, y = ctDNAfraction)) +
  ...
  geom_smooth(method = "lm", se = TRUE, color = "#3E3EA3") +  # regression line without standard error shading
  annotate("text", x = 0.2, y = Inf, label = cor_label, hjust = 0, vjust = 1.5, size = 5, color = "black") +  # Add correlation stats
...
add not detected dots fig5, empty circles
also add stage III to see
maybe the volume not correlate
```{r}
volume_plot <- demografics %>%
  mutate(Stage_group = case_when(
           grepl("^I[A-B]?$", Stage_diagnosis) ~ "I",
...
combined_plot <- (p_list[[1]] + p_list[[2]] )/  # First row: plot1 and plot2 side by side
  ( plot_spacer() + p_list[[3]] + plot_spacer()) +  # Second row: plot3 centered
  plot_layout(nrow = 2,ncol=1, heights = c(1, 1))  # Define layout with 2 rows
combined_plot
...
Fig. S3: 
### (A) Correlation between ctDNA and tumour volume in the present study and recently published reports. While all studies show a significant correlation between ctDNA and tumour volume, the present study also detected and quantified ctDNA fractions in patients with tumour volumes below 1 cm3. 
```{r}
