...
#### already updated in Apr02.

<!-- # The tumour mutations number might need to update based on the mutList in the table 7. -->

<!-- ```{r} -->

<!-- update_sum <- mutLists%>%group_by(Patient)%>%summarise(update_Tumour_mutations_number=n()) -->

<!-- demografics<-demografics %>% left_join(update_sum,by="Patient") -->

<!-- print(demografics %>% select(Patient,Tumour_mutations_number,update_Tumour_mutations_number)) -->
...
<!-- ### panel design result summary -->

<!-- ```{bash} -->

<!-- for file in KHdata/panel_design_results/batch*/*Covered.bed -->

<!-- do  -->

<!-- lines=$(grep -R "^chr" $file | wc -l);  -->

<!-- echo $file covered loci : $lines ;  -->
...
ggplot(data=demografics, aes(x=Stage_diagnosis, fill=Histological_subtype)) +
...
## (C) Subtype distribution in the LUCID cohort.
...
ggplot(subtype_counts, aes(x = "", y = n, fill = Histological_subtype)) +
...
# Fig. 2: Analysis of tumour whole exome sequencing data in NSCLC.

## (A) Genes commonly mutated in adenocarcinoma were identified as being significantly mutated from Cancer Genome Atlas (CGA) analysis (19). Mutations in these genes were identified in the present cohort (filled boxes) from whole exome sequencing data, for patients with adenocarcinoma and where tumour tissue WES was available (n=55 patients).
...
p1<- ggplot(df_plot, aes(x = Patient, y = Label, fill = factor(Value))) +
...
## (B) Genes commonly mutated in squamous cell carcinoma were identified as significantly mutated from CGA analysis (20). Mutations in these genes were identified in the present cohort (filled boxes) from whole exome sequencing data, for patients with squamous cell carcinoma and where tumour tissue WES was available (n=28 patients).
...
p2<-ggplot(df_plot, aes(x = Patient, y =  Label, fill = factor(Value))) +
  geom_tile(color = "black") +  # black grid lines
  scale_fill_manual(values = c("0" = "white", "1" = "#3FAB3E")) +
  scale_x_discrete(position = "top") +  # Moves the x axis to the top
...
## (C) Mutation signatures were obtained using deconstructSigs (22) and their presence is shown (filled boxes) for the 90 patients in this study from whole exome sequencing data, for the patients where tissue WES data was available.

### Signature annotation using signatures.nature2013

```{r}
# # formatting mutations
# sigs.input <- mut.to.sigs.input(mut.ref = mutLists, 
#                                 sample.id = "Patient", 
#                                 chr = "Chr", 
#                                 pos = "Pos", 
#                                 ref = "Ref", 
#                                 alt = "Alt")
# patient_IDs <- unique(mutLists$Patient)
# signature_summary <- c()
# # check the signatures
# for (k in 1:length(patient_IDs)) {
#   # print(paste0("working on ", patient_IDs[k]))
#   curr_file = whichSignatures(tumor.ref = sigs.input, 
#                            signatures.ref = signatures.nature2013, # this can be changed between signatures.nature2013 and signatures.cosmic
#                            sample.id = patient_IDs[k], 
#                            contexts.needed = TRUE,
#                            tri.counts.method = 'exome')
#   # plots for each patient
#   pdf(paste0(output_plot_dir, "signature/bar_and_pie/bar_", patient_IDs[k], "_signature.pdf"))
#   plotSignatures(curr_file, sub = 'example')
#   dev.off()
#   
#   pdf(paste0(output_plot_dir, "signature/bar_and_pie/pie_", patient_IDs[k], "_signature.pdf"))
#   makePie(curr_file, sub = 'example')
#   dev.off()
# 
#   curr_summary <- as.data.frame(curr_file["weights"])
#   curr_summary <- data.frame(t(curr_summary))
#   curr_summary$sample <- patient_IDs[k]
#   curr_summary$signature <- rownames(curr_summary)
#   rownames(curr_summary) <- 1:nrow(curr_summary)
#   colnames(curr_summary) <- c("fraction", "sample", "signature")
#   # all patients' signatures
#   signature_summary <- rbind(signature_summary, curr_summary)
# }
# write.csv(signature_summary, paste0(output_plot_dir, "../signature_summary.csv"), row.names = F)

# load signature annotation table
signature_summary<-read.csv(paste0(output_plot_dir, "../signature_summary.csv"))
```

### Generate heatmap plot

p3 delete. no need anymore

```{r}
# # formatting table
# df_plot <- signature_summary %>%
#   # Remove the prefix from the signature column.
#   mutate(Signature = gsub("weights\\.Signature\\.", "Signature ", signature)) %>%
#   # Create a new Value column: 1 if fraction > 0, else 0.
#   mutate(Value = ifelse(fraction > 0, 1, 0)) %>%
#   # Rename 'sample' column to 'Patient' and select only the desired columns.
#   mutate(Patient = sample) %>%
#   select(Patient, Signature, Value)
# 
# sig_percents <- df_plot %>%
#   group_by(Signature) %>%
#   summarise(perc = round(sum(Value) / n() , 4)*100) %>%
#   mutate(Label = paste0(Signature, " (", perc, "%)")) %>%
#   filter(perc > 0) %>% 
#   arrange(perc)
# 
# df_plot <- df_plot %>%
#   inner_join(sig_percents %>% select(Signature, Label), by = "Signature")
# 
# top_label <- sig_percents %>% filter(perc==max(perc))%>% select(Label)
# top_label <- top_label$Label[1]
# # For that gene, order patients so that Value = 0 patients come first, then Value = 1.
# top_patient_order <- df_plot %>%
#   filter(Label == top_label) %>%
#   arrange(Value, Patient) %>%  # First sort by Value (0 then 1), then by Patient (if needed)
#   pull(Patient) %>%
#   unique()
# 
# # Apply the new ordering to the Patient factor in the entire dataset.
# df_plot <- df_plot %>%
#   mutate(Patient = factor(Patient, levels = top_patient_order))
# 
# 
# p3 <- ggplot(df_plot, aes(x = Patient, y =  factor(Label, levels = sig_percents$Label), fill = factor(Value))) +
#   geom_tile(color = "black") +  # black grid lines
#   scale_fill_manual(values = c("0" = "white", "1" = "#3FAB3E")) +
#   scale_x_discrete(position = "top") +  # Moves the x axis to the top
#   labs(x = "", y = "Signature", fill = "Mutation",subtitle="Patient") +
#   theme_minimal() +
#   theme(
#     plot.subtitle = element_text(hjust = 0.5,vjust=-5,size=13),    # Center the title at the top
#     axis.text.x = element_blank(),            # Remove x-axis labels (patient IDs)
#     axis.ticks.x.top = element_blank(),           # Remove x-axis ticks
#     legend.position = "none",                  # Remove legend
#     axis.line.y = element_line(color = "black"), # add x axis line
...
## (D) A significant correlation was observed between the tumour mutation burden and the extent of smoking history (Pearson’s R = 0.38, p = 0.00023).

```{r}
# #### PRIVATE CLINICAL DATA!
# KH_CCTC <- read.csv("KHdata/CCTC_output/Main datasets_4/Inivata_INVAR_95BLcomb.csv")
# KH_CCTC <- KH_CCTC %>% filter(!is.na(SMOKEYEARS))%>% 
#   mutate(Patient=as.character(Patient))%>%
#   mutate(Patient= paste0(2,substr(Patient, 4,4),substr(Patient, 3,3)))%>%
#   select(Patient,SMOKEYEARS)
# ### END!
# 
# # smoke plot data
# smoke_plot <- demografics %>%
#   left_join(KH_CCTC,by="Patient")%>%
#   mutate(SMOKEYEARS = ifelse(Smoking_history=="Never Smoked",0,SMOKEYEARS))%>%
#   # mutate(update_Tumour_mutations_number = Tumour_mutations_number) %>% # old version
#   filter(!is.na(update_Tumour_mutations_number),!is.na(Smoking_history),!is.na(SMOKEYEARS))%>%
#   mutate(update_Tumour_mutations_number= as.integer(update_Tumour_mutations_number),
#          SMOKEYEARS=as.integer(SMOKEYEARS))%>%
#   mutate(old_smoke_months = SMOKEYEARS*12)
# 
# # Calculate correlation and p-value in log scale
# cor_test <- cor.test(smoke_plot$old_smoke_months, smoke_plot$update_Tumour_mutations_number, method = "pearson")
# #cor_test <- cor.test(volume_plot$T_volume_cm3, volume_plot$ctDNAfraction, method = "pearson")
# cor_label <- paste0("R = ", round(cor_test$estimate, 2), ", p = ", round(cor_test$p.value,8))
# 
# p4 <- ggplot(smoke_plot, aes(x = old_smoke_months, y = update_Tumour_mutations_number)) +
#   # Plot detected points (green)
#   geom_point(
#     data = smoke_plot[smoke_plot$Smoking_history=="Current Smoker", ],
...
#   geom_point(
#     data = smoke_plot[smoke_plot$Smoking_history=="Ex-smoker", ],
#     aes(fill = Smoking_history),
#     shape = 24,
#     size = 2,
#     color = "#2ca02c"
#   ) +
#   geom_point(
#     data = smoke_plot[smoke_plot$Smoking_history=="Never Smoked", ],
#     aes(fill = Smoking_history),
#     shape = 22,
#     size = 2,
#     color = "#8da2df"
...
#   annotate("text", x = 430, y = 2600, label = cor_label, hjust = 1, vjust = 1, color = "black",size=5) +  # Add correlation stats
#   labs(
#     x = "Months smoking",
#     y = "Tumour mutations identified \n by whole exome sequencing",
...
p_list[[i]] <-  p_list[[i]] +
  ...
  theme(plot.title = element_text(size = 12, face = "bold", hjust = -0.4, vjust = 0))  # Position 
}
...
ggsave(
  filename = "new_plots_CZ/Fig2AB_plot.png",  # File name
...
# 
# ggsave(
#   filename = "new_plots_CZ/Fig2C_plot.png",  # File name
...
# Fig. 3: ctDNA detection in early-stage NSCLC by patient-specific sequencing panels and INVAR analysis.

## (A) Specificities for the three custom capture sequencing panels in this study. Specificities were assessed twice. Once using other (non-matched) patients as controls, and once using an independent set of healthy controls.
...
## (B) Informative Reads (IR) are plotted against the Integrated Mutant Allele Fraction (IMAF) which estimates the ctDNA levels in each sample. Sensitivity of the INVAR pipeline increases with increasing IR and the threshold for ctDNA detection can be roughly estimated as 1/IR (indicated by dashed diagonal line). Samples with less than 20,000 IR (dark grey box) allow only for limited detection sensitivity. Samples in which ctDNA was detected by INVAR are shown in green. Undetected samples are shown in grey.
...
p1<-ggplot(detection_data, aes(x = IR, y = plot_IMAF)) +
...
## (C) Cancer genomes detected in the analysis of plasma samples. The total number of cancer genomes represented in each sample analysed was estimated for patients with detected ctDNA. In many samples, ctDNA was detected even though less than 0.1 of a single cancer genome was present, and would likely not be detected without the analysis of a greater number of confirmed markers.

```{r}
# cancer genomes vs ctDNA fraction (IMAF)
# what does cancer genomes mean? range from ND - 0.01 - 10
# one genome size is 3Gb, calculate one genome weight = (one base pair g/mol)/(Avogadro constant mol^-1) * genome size = 3.3pg
# need 1000 genome copy, so put in 3.3ng DNA for library, so number of cancer genome is 1000 * IMAF
...
p2<-ggplot(detection_data, aes(x = PATIENT)) +
...
## (D) ctDNA tumour fractions, in parts per million (ppm), detected in baseline plasma samples according to disease stage at diagnosis, for the 90 patients analysed with INVAR (55, 18 and 17 patients with stage I, II and III disease respectively). Median ctDNA levels per stage are indicated with vertical dotted lines.
...
p_list <- list(p1,p2,p3[[1]],p3[[2]],p3[[3]]) 
for(i in c(1:3)){
p_list[[i]] <-  p_list[[i]] +
  labs(title = subtitle_lab[i]) +  # Add label "A"
...
design <- c(
  ...
  area(t = 1, l = 1, b = 1, r = 1),  # Plot B (Row 1, Column 2)
  area(t = 1, l = 2, b = 1, r = 2),  # Plot B (Row 1, Column 2)
  area(t = 2, l = 1, b = 2, r = 2),  # Plot C (Row 2, spans all columns)
  area(t = 3, l = 1, b = 3, r = 2),   # Plot D (Row 3, spans all columns)
  area(t = 4, l = 1, b = 4, r = 2),   # Plot D (Row 3, spans all columns)
  area(t = 5, l = 1, b = 5, r = 2)   # Plot D (Row 3, spans all columns)
)
combined_plot<-p_list[[1]]+ plot_spacer()+p_list[[2]] + p_list[[3]] + p_list[[4]] + p_list[[5]] + plot_layout(design = design,heights =c(4,5,1,1,1),widths = c(2,1,4,4,4,4))


ggsave(
  filename = paste0("new_plots_CZ/Fig3","BCD","_plot.png"),  # File name
  plot = combined_plot,            # The plot object to save
  width = 10,                      # Width in inches
...
# Fig. 4: InVisionFirst®-Lung assay analysis in 27 patients with NSCLC.

## (A) A total of 27 mutations were identified; most alterations were identified in TP53 (37%), followed by KRAS (22%). Where matched tissue data was available, we confirmed 70% (14/20) of these mutations. Of the remaining 6 mutations, one was confirmed to be a germline variant, two splice variants were not covered in our analysis, and three were not detected in tumour whole exome sequencing.

keep original plot. But need to change the gene names to small right?

```{r}

```

## Fig 4B in the excel file (Fig4b)
...
## (C) ctDNA fraction correlation between INVAR and InVisionFirst®-Lung. 10 samples were detected by both platforms; yielding a correlation in ctDNA fraction of 0.92 (Spearman’s r, p = 4.7x10-4).

THere is no problem with this plot. The previous one has a case that both AF=0, but the new result doesn't have such case
...
df_plot <- demografics %>% 
  ...
  select(Patient,INVAR2_fraction_IMAF,InVisionFirst..Lung_mean_AF,Detection_either)
...
ggsave(
  filename = "new_plots_CZ/Fig4C_plot.png",  # File name
  plot = p1,            # The plot object to save
  width = 4,                      # Width in inches
  height = 3.9,                     # Height in inches
  dpi = 300,                       # Resolution (dots per inch)
  units = "in"                     # Units of width and height
)
p1
...
If mark the both not detected as empty circle?

```{r}
ggplot(df_plot%>%filter(Detection_either==T), aes(x = InVisionFirst..Lung_mean_AF, y = INVAR2_fraction_IMAF,group=Detection_either)) +
  geom_point(color = "#00876c",size=3) +  # Green dots
  geom_point(data = df_plot%>%filter(Detection_either==F), color = "red",size=3,shape=1) +  # Green dots
  labs(
...
# Fig. 5: Summary of ctDNA levels and patient demographics.

## (A) In total, ctDNA was detected in 66 of 100 samples by at least one of the two analysis approaches. Patients are ordered by stage at diagnosis and detection/ctDNA levels; patient demographics are indicated below their respective ctDNA fraction. Detection rates are reported in the text.
...
# save image
png("new_plots_CZ/Fig5A_ctdna_heatmap.png", width = 8.5, height = 6, units = "in", res = 300)
...
```{r}
# Don't know is only INVAR or combine both INVAR and Invision. If combined, do we choose the highest ctDNA fraction for each patient?
## 1. only INVAR result
box_plot <- demografics %>% 
  mutate(INVAR2_fraction_IMAF=as.numeric(INVAR2_fraction_IMAF)) %>%
  mutate(Stage_group = case_when(
           grepl("^I[A-B]?$", Stage_diagnosis) ~ "I",
           grepl("^II[A-B]?$", Stage_diagnosis) ~ "II",
           grepl("^III[A-B]?$", Stage_diagnosis) ~ "III",
           TRUE ~ NA_character_  )) %>%
  select(Patient,Stage_group,INVAR2_fraction_IMAF,INVAR2_detection)
# total patients in each stage
total_stage <- box_plot%>% group_by(Stage_group)%>%summarise(total=n())
# filter detected patients
box_plot <- box_plot %>% filter(INVAR2_detection=="Yes")
...
```{r}
# 2. use the max AF of both INVAR and Invision and choose the highest ctDNA fraction bettween these two.
box_plot <- demografics %>%
    mutate(
...
## (C) Correlation between ctDNA and tumour volume for patients with stage I disease (red dots) and stage II disease (turquoise triangles) in cases where ctDNA was detected and tumour measurements were available (n=42). Pearson’s R = 0.62, p = 1x10-5.
...
ggsave(
  filename = "new_plots_CZ/Fig5BC_plot.png",  # File name
  plot = combined_plot,            # The plot object to save
  width = 10,                      # Width in inches
  height = 5,                     # Height in inches
  dpi = 300,                       # Resolution (dots per inch)
...
add not detected dots fig5, empty circles also add stage III to see maybe the volume not correlate
...
p1<-ggplot(volume_plot_detect, aes(x = T_volume_cm3, y = ctDNAfraction)) +
  ...
  labs(
    x = "Tumor volume cm3",
    y = "ctDNA fraction",
    title = "In log scale for correlation test for the detected only"
  ) +
  scale_x_log10(
    limits = c(0.2, 600),
    breaks = c(1, 10, 100)
...
ggsave(
  filename = "new_plots_CZ/Fig5D+stage3_plot.png",  # File name
  plot = p1,            # The plot object to save
  width = 7,                      # Width in inches
...
# Fig. 6: Analysis of patient outcomes and ctDNA detection

Kaplan-Meier plots showing overall survival (OS) and the time that patients remained free from relapse of the original primary tumour (FFR). FFR analysis considers as events only clinical relapse of the original primary tumour, and patient data is right-censored when patients present with a second primary cancer that progresses or upon death from causes other than relapse of the first primary disease. Analyses of cancer-free survival considering all cancer or death events (CFS), and additional OS analysis, are presented in fig. S4A and B) Comparison of FFR time (A) and OS (B) for patients who underwent non-surgical treatment (red, dotted) vs. surgical treatment (blue). Median FFR time of 578 days vs. not reached (Log-Rank Test p-value \<0.0001); HR for non-surgical treatment: 4.35, p-value \<0.0001). Median OS time of 893 days vs. not reached (Log-Rank Test p-value 0.002); HR for non-surgical treatment: 2.71, p-value 0.002). C+D) Comparison of FFR time (C) and OS (D) for patients with ctDNA detected (red, dotted) vs. not detected (blue). Median FFR time of 990 days vs. not reached (Log-Rank Test p-value 0.001); HR for ctDNA detected: 5.20, p-value 0.002). Median OS time not reached for both (Log-Rank Test p-value 0.036); HR for ctDNA detected: 2.25, p-value 0.042. E) Comparison of FFR time for patients who underwent surgery, with ctDNA detected (red, dotted) vs. not detected (blue). Median FFR time not reached for both (Log-Rank Test p-value 0.004); HR for ctDNA detected: 11.02, p-value 0.021). F) Comparison of FFR time for patients with adenocarcinoma, with ctDNA detected (red, dotted) vs. not detected (blue). Median FFR time of 686 days vs. not reached (Log-Rank Test p-value 0.0003); HR for ctDNA detected: 9.34, p-value 0.003). See fig. S4 for additional OS and CFS comparisons.
...
make_survplot<-function(surv_data,legend_labs,fit,ylabel,subtitle){
  ...
  ggtheme = theme_survminer()+theme(
      ...
      plot.title = element_text(hjust = -0.04, size = 20, face = "bold")  # Move title further left
    ),
...
# subplot EF
survival_plot_E <- survival_plot%>%filter(Treatment_category=="Surgical (with or without adjuvant treatment)")
surv_object <- Surv(time = survival_plot_E$FFR_date, event = survival_plot_E$FFR_event)
fit <- survfit(surv_object ~ ctDNA, data = survival_plot_E)
median_survs<- summary(fit)$table[,"median"] 
cox_fit <- coxph(surv_object ~ ctDNA, data = survival_plot_E)
HR<-round(summary(cox_fit)$coefficients[,c("exp(coef)","Pr(>|z|)")],3)
median_hr[5,]<-c(median_survs,HR)
p[[5]]<-make_survplot(survival_plot_E,legend_labs=levels(survival_plot_E$ctDNA),fit,y_labels[1],subtitle_lab[5])

survival_plot_F<- survival_plot%>%filter(Histological_subtype=="Adenocarcinoma")
surv_object <- Surv(time = survival_plot_F$FFR_date, event = survival_plot_F$FFR_event)
fit <- survfit(surv_object ~ ctDNA, data = survival_plot_F)
median_survs<- summary(fit)$table[,"median"] 
cox_fit <- coxph(surv_object ~ ctDNA, data = survival_plot_F)
HR<-round(summary(cox_fit)$coefficients[,c("exp(coef)","Pr(>|z|)")],3)
median_hr[6,]<-c(median_survs,HR)
p[[6]]<-make_survplot(survival_plot_F,legend_labs=levels(survival_plot_F$ctDNA),fit,y_labels[1],subtitle_lab[6])

combined_plot <- p[[1]]+p[[2]]+p[[3]]+p[[4]]+p[[5]]+p[[6]]+
  plot_layout(nrow=3,ncol=2, heights = c(1, 1, 1), widths = c(1, 1))

combined_plot

ggsave(
  filename = "new_plots_CZ/survival_plot_Fig6.png",  # File name
  plot = combined_plot,            # The plot object to save
  width = 24,                      # Width in inches (adjust as needed)
  height = 16,                      # Height in inches (adjust as needed)
  dpi = 300,                       # Resolution (dots per inch)
  units = "in"                     # Units of width and height
)
...
# change to all ctDNA fraction \> 0 instead of detection==T
...
for(i in 1:4){
  ...
  if(i %in% c(1,2)){
    fit <- survfit(surv_object ~ Treatment_category, data = survival_plot_all_IMAF)
    p[[i]] <- make_survplot(survival_plot_all_IMAF,levels(survival_plot_all_IMAF$Treatment_category),fit,ylabel,subtitle_lab[i])
...
# subplot EF
survival_plot_E <- survival_plot_all_IMAF%>%filter(Treatment_category=="Surgical (with or without adjuvant treatment)")
surv_object <- Surv(time = survival_plot_E$FFR_date, event = survival_plot_E$FFR_event)
fit <- survfit(surv_object ~ ctDNA, data = survival_plot_E)
p[[5]]<-make_survplot(survival_plot_E,legend_labs=levels(survival_plot_E$ctDNA),fit,y_labels[1],subtitle_lab[5])

survival_plot_F<- survival_plot_all_IMAF%>%filter(Histological_subtype=="Adenocarcinoma")
surv_object <- Surv(time = survival_plot_F$FFR_date, event = survival_plot_F$FFR_event)
fit <- survfit(surv_object ~ ctDNA, data = survival_plot_F)
p[[6]]<-make_survplot(survival_plot_F,legend_labs=levels(survival_plot_F$ctDNA),fit,y_labels[1],subtitle_lab[6])
...
ggsave(
  filename = "new_plots_CZ/survival_plot_IMAF+_Fig6.png",  # File name
  plot = combined_plot,            # The plot object to save
  width = 24,                      # Width in inches (adjust as needed)
  height = 16,                      # Height in inches (adjust as needed)
  dpi = 300,                       # Resolution (dots per inch)
...
### (A) Distribution of the number of mutations identified in exome-wide sequencing of tumour samples, for patients with different smoking status/history. Current and former smokers had significantly more mutations identified in their tumour tissue samples compared to never smokers.

### (B) Tumour mutations per patient plotted against the age of the patient. No significant correlation was observed.
...
### (C) The fraction of plasma samples with ctDNA detected out of all 100 patients in the study, when different thresholds for detection are imposed ranging from ctDNA fraction of 1x10-6 (which includes all samples detected in this study) to fractional concentration of 1x10-2. Detection rates decreased with increasing thresholds; this decrease was most notable up to ctDNA fraction of 1x10-3 for patients with early-stage disease.
...
#  For now, I use the Detection_either result as y-axis result, (even though IMAF>0, the detection=FALSE )
bar_sum <- bar_plot%>%group_by(Stage_group)%>%
  summarise(total=n(), 
            "1e-06"= sum(ctDNAfraction>1e-6 & Detection_either== TRUE)/total, 
            "1e-05"= sum(ctDNAfraction>1e-5 & Detection_either== TRUE)/total, 
            "1e-04"= sum(ctDNAfraction>1e-4 & Detection_either== TRUE)/total,
            "1e-03"= sum(ctDNAfraction>1e-3 & Detection_either== TRUE)/total,
            "1e-02"= sum(ctDNAfraction>1e-2 & Detection_either== TRUE)/total) %>%
  select(-total,-`1e-06`) # because there is only one 1e06 sample and it is not detected. so 1e-5 and 1e-6 have the same detection rate.

# Convert to long format
bar_sum_long <- bar_sum %>%
  pivot_longer(
    cols = -Stage_group,  # Keep Stage_group as the identifier
    names_to = "Threshold",  # Column for the threshold values (1e-05, 1e-45, etc.)
    values_to = "Detected_Fraction"  # Column for the detected fraction values
  ) %>% mutate(Threshold = factor(Threshold,levels = c("1e-05", "1e-04", "1e-03", "1e-02")))
...
design <- c(
  #area(t = 1, l = 1, b = 1, r = 1),  # Plot B (Row 1, Column 2)
  area(t = 1, l = 1, b = 1, r = 2),  # Plot B (Row 1, Column 2)
  area(t = 1, l = 3, b = 1, r = 4),  # Plot B (Row 1, Column 2)
  area(t = 2, l = 1, b = 2, r = 1),
  area(t = 2, l = 2, b = 2, r = 3),
  area(t = 2, l = 4, b = 2, r = 4)
)
combined_plot<-p_list[[1]]+ p_list[[2]]  +plot_spacer()+ p_list[[3]] + plot_spacer() +  plot_layout(design = design,heights =c(4,4,4,4,4),widths = c(2,2,2,2,2))

combined_plot

ggsave(
  filename = "new_plots_CZ/FigS2_plot.png",  # File name
  plot = combined_plot,            # The plot object to save
...
Fig. S3: \### (A) Correlation between ctDNA and tumour volume in the present study and recently published reports. While all studies show a significant correlation between ctDNA and tumour volume, the present study also detected and quantified ctDNA fractions in patients with tumour volumes below 1 cm3.

```{r}
# tumor volume of others study. find Katerin's code
...
#### ctDNA fraction use max from Invision and INVAR2. Correlation test is in log scale.
...
ggsave(
  filename = "new_plots_CZ/FigS3B.png",  # File name
...
## Fig. S4: Kaplan-Meier plots showing overall survival (OS) and cancer-free survival considering all cancer or death events (CFS).
...
# subplot A
    ...
    fit <- survfit(surv_object ~ Treatment_category, data = survival_plot)
    p[[1]] <- make_survplot(survival_plot,levels(survival_plot$Treatment_category),fit,y_labels[1],subtitle_lab[1])
...
# subplot B
    surv_object <- Surv(time = survival_plot$CFS_date, event = survival_plot$CFS_event)
    fit <- survfit(surv_object ~ ctDNA, data = survival_plot)
    p[[2]] <- make_survplot(survival_plot,levels(survival_plot$ctDNA),fit,y_labels[1],subtitle_lab[2])
...
# subplot C
    ...
    p[[3]] <- make_survplot(survival_plot_surgic,legend_labs=levels(survival_plot_surgic$ctDNA),fit,y_labels[1],subtitle_lab[3])
...
# subplot E
    ...
    fit <- survfit(surv_object ~ ctDNA, data = survival_plot_surgic)
    p[[5]] <- make_survplot(survival_plot_surgic,legend_labs=levels(survival_plot_surgic$ctDNA),fit,y_labels[2],subtitle_lab[5])
...
# subplot F
    ...
    fit <- survfit(surv_object ~ ctDNA, data = survival_plot_adeno)
    p[[6]]<-make_survplot(survival_plot_adeno,legend_labs=levels(survival_plot_adeno$ctDNA),fit,y_labels[2],subtitle_lab[6])
...
ggsave(
  filename = "new_plots_CZ/Supp_survival_plot_FigS4.png",  # File name
  plot = combined_plot,            # The plot object to save
  width = 24,                      # Width in inches (adjust as needed)
  height = 16,                      # Height in inches (adjust as needed)
  dpi = 300,                       # Resolution (dots per inch)
...
IMAF\>0 instead of detection just to see
...
rownames(median_hr)<-subtitle_lab
# subplot A
    surv_object <- Surv(time = survival_plot_all_IMAF$CFS_date, event = survival_plot_all_IMAF$CFS_event)
    fit <- survfit(surv_object ~ Treatment_category, data = survival_plot_all_IMAF)
    p[[1]] <- make_survplot(survival_plot_all_IMAF,levels(survival_plot_all_IMAF$Treatment_category),fit,y_labels[1],subtitle_lab[1])

    median_survs<- summary(fit)$table[,"median"] 
    cox_fit <- coxph(surv_object ~ Treatment_category, data = survival_plot_all_IMAF)
    HR<-round(summary(cox_fit)$coefficients[,c("exp(coef)","Pr(>|z|)")],3)
    median_hr[1,]<-c(median_survs,HR)
...
# subplot B
    surv_object <- Surv(time = survival_plot_all_IMAF$CFS_date, event = survival_plot_all_IMAF$CFS_event)
    fit <- survfit(surv_object ~ ctDNA, data = survival_plot_all_IMAF)
    p[[2]] <- make_survplot(survival_plot_all_IMAF,levels(survival_plot_all_IMAF$ctDNA),fit,y_labels[1],subtitle_lab[2])
...
# subplot C
    survival_plot_surgic <- survival_plot_all_IMAF%>%filter(Treatment_category=="Surgical (with or without adjuvant treatment)")
    surv_object <- Surv(time = survival_plot_surgic$CFS_date, event = survival_plot_surgic$CFS_event)
    fit <- survfit(surv_object ~ ctDNA, data = survival_plot_surgic)
    p[[3]] <- make_survplot(survival_plot_surgic,legend_labs=levels(survival_plot_surgic$ctDNA),fit,y_labels[1],subtitle_lab[3])
...
# subplot D
    survival_plot_adeno <- survival_plot_all_IMAF%>%filter(Histological_subtype=="Adenocarcinoma")
    surv_object <- Surv(time = survival_plot_adeno$CFS_date, event = survival_plot_adeno$CFS_event)
    fit <- survfit(surv_object ~ ctDNA, data = survival_plot_adeno)
    p[[4]]<-make_survplot(survival_plot_adeno,legend_labs=levels(survival_plot_adeno$ctDNA),fit,y_labels[1],subtitle_lab[4])
...
ggsave(
  filename = "new_plots_CZ/Supp_survival_plot_IMAF+_FigS4.png",  # File name
  plot = combined_plot,            # The plot object to save
  width = 24,                      # Width in inches (adjust as needed)
  height = 16,                      # Height in inches (adjust as needed)
  dpi = 300,                       # Resolution (dots per inch)
